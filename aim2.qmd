...
In our urbanicity-specific simulation, we again incremented by 50 from -200 to 200. In the plot below, we see the relationships between BIC score and $d_{ij}$ for each migration type while holding $d_{ij}$ constant at 0 for all other migration types. For migration to rural destinations (represented by orange $d_{rr}$ and red $d_{ru}$), the minimum BIC score occurs when there is no difference in health between origin and destination. However, for migration to urban destinations, there is some evidence of lower BIC scores when urban origins have lower age-adjusted premature mortality rates than their correpsonding urban destinations (represented by $d_{uu}$). In this case, movers tend to migrate from healthier urban counties to less healthy urban counties.
...
Based on the trends shown in the plot above, we narrowed our search increment to 20 and tested a specific range of \$d\_{uu}\$ values to account for a -140 to 20 difference in age-adjusted premature mortality rates between urban origin and urban destination. For all other types of migration, we used the same range. Therefore, we tested \$d\_{ur}\$, \$d\_{rr}\$, and \$d\_{ru}\$ values with a range of -80 to 80 difference between origin and destination. This resulted in an additional 6,561 unique sets of $(d_{uu}, d_{rr}, d_{ru}, d_{ur})$. <!--# this is the same wording as in the methods section...  -->The results of this narrowed search are shown below:
