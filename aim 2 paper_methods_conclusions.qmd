...
$$  y_{it} = \beta_0 + \sum_{k=1}^{K} \beta_{1k} \cdot \text{ft}_{k} + \beta_{2}y_{t-1,i} + \mu_{i} + \epsilon_{it}    $$
...
$u_{it}$ is the spatially independent error term for each county $i$ at year $t$
...
Thus, the model we determined to have most explanatory power is as follows:

$$  y_{it} = \beta_0 + \sum_{k=1}^{K} \beta_{1k} \cdot \text{ft}_{k} + \sum_{k=1}^{K} \beta_{2k} ns(y_{t-1,i}, 4) \cdot r_i^{k} + \sum_{k=1}^{K} \beta_{3k} ns(migterm_{it}, 4) \cdot r_i^{k} + \sum_{k=1}^{K} \beta_{4k} r_i^{k} + \mu_i + \epsilon_{it} + \lambda W \epsilon_{it} + u_{it} $$
...
$ns(y_{t-1,i}, 4)$ is a natural spline transformation of the lagged premature age-adjusted mortality rate with 4 degrees of freedom\
$ns(migterm_{it}, 4)$ is a natural spline transformation of the migration term with 4 degrees of freedom\
...
In the plot below, we see $d_{ur}$ and $d_{ru}$ plotted versus BIC score while holding the other constant at the value that produces the minimum. The trend in BIC score is primarily driven by $d_{ru}$, suggesting that migration from urban origins to rural destinations primarily drives destination health.
...
bicmin_kru <- bicktot %>% filter(kuu == mostex$kuu & krr == mostex$krr & kur == mostex$kru)
bicmin_kur <- bicktot %>% filter(kru == mostex$kru & kuu == mostex$kuu & krr == mostex$krr)
