...
We began with a broad increment of 50, testing values of $d_{ij}$ ranging from -200 to 200 premature deaths per 100,000 higher than the destination county. This initial search tested a total of 6,561 unique sets of $(d_{uu}, d_{rr}, d_{ru}, d_{ur})$. After identifying a potential range of $d_{ij}$ values that minimized the BIC score, we refined our search. Using a smaller increment of 10, we tested values of $d_{ij}$ within a more precise range of -40 to 40 premature deaths per 100,000 higher than the destination county, resulting in another 6,561 unique sets of $(d_{uu}, d_{rr}, d_{ru}, d_{ur})$.
...
bicmin = bicktot %>% filter(kuu == mostex$kuu & krr == mostex$krr)
...
# note that in this plot, when the xaxis is 0, 50, and -50, the range of points is larger because the range of kur is larger since those kru values were tested in the initial grid and the restricted grid 

# when dur and dru are positive, bic scores are lower. but when dur is positive and dru is negative, bic scores are higher. 
...
ggplot(aes(x = as.numeric(kur), y = as.numeric(bicspat), color = as.factor(colorvar)), data = bicmin) +
  geom_point(alpha = 0.3) +
  xlab(expression(d[ru])) +
  ylab("BIC") +
  theme_bw() +
...
bicmin = bicktot %>% filter(kur == 0 & kru == 0)
...
plot(x = bicmin$kur %>% filter(bicmin)
```
...
bicmin = bicktot %>% filter(kur == mostex$kur & kru == mostex$kru)

plot(x = bicmin$krr, y = bicmin$kuu)

bicmin = bicktot %>% filter(kuu == 0 & krr == 0)
bicmin$colorvar = ifelse(bicmin$kur >0, 1, 0)
