...
```{r #exploration of weird BIC scores (w 4df, hopefully fixed???) }
...
summary(bicktot$bicspat)

weirdbic = bicktot %>% filter(bicspat>360000)

ggplot(weirdbic, aes(x = krr, y = bicspat)) + geom_point()
```
...
```{r plot with separate kuu, krr, kur, kru held at 0 , broad search }
#load("data_processed/bick_restrict.Rdata")
load("data_processed/bickdf.Rdata")
