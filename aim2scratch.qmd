...
<!--# hypotheses should be at end of intro instead -->
...
<!--# make more clear that migration term is added next -->
...
<!--# this goes in the results section -->
...
<!--# this goes in the model fitting section; no equations in results; and no data in method!! table1 is first thign in results -->
...
Our outcome of interest, county-level premature age-adjusted mortality, has both strengths and limitations. We chose to use premature age-adjusted mortality for its comparability across years and locations and its consistent, robust definition. Despite this, the 2019 median age of movers in the US was under 30 years old [@s0701:g], indicating that the migrating population is not necessarily the same population that is dying. Additionally, mortality in the US primarily results from chronic diseases with long latency periods between exposure/onset and death. \<!#--cite–\> This discrepancy suggests that trends in county-level premature mortality might be better explained by migration trends from twenty to fifty years earlier. However, in a study of tuberculosis cases among immigrants to the Netherlands, van Aart and colleagues found an approximately 17-month lag in tuberculosis cases among Somali immigrants, but no lag among immigrants from Morocco and Turkey, suggesting measurable differences in health among immigrants should be apparent within a one-year window [@vanaart2017]. Though our analyses do not include infectious disease outcomes, we apply the same logic. Nonetheless, county-level measures of premature mortality are widely used as a proxy for overall county health. For example, a 1983 WHO report recommended mortality and morbidity measures to understand health and migration [@gushulak2006], and length of life measures hold 50% weight in the County Health Rankings Model of place-based health outcomes [@explore]. Ideally, we would use age-group-specific measures of mortality and migration, but this would require data beyond the IRS migration flow data used here and would preclude analysis of the entire United States. Additionally, assessing migrant health directly would require individual longitudinal data, which is not within the scope of this project.
